...
:- dynamic (neg)/1, (:=)/2, (:^)/2.
:- multifile (neg)/1, (:=)/2, (:^)/2.
:-set_ prolog_flag(unknown,_,fail).
%
% @(+Goal)
...
not_rebutted(KB,(Goal :- Condition)).
def_der(KB,Goal) :-
% A goal is defeasibly derivable if it is
...
def_der(KB,Condition),
not_rebutted(KB,(Goal := Condition)),
not_undercut(KB,(Goal := Condition)).
def_der(KB,Goal) :-
preemption,
...
%
% not_undercut(+KB,+Rule)
% Succeeds if \+ undercut(+KB,Rule) succeeds. This
% predicate is added to introduce a cut in a way that
% reduces the number of duplicate solutions.
%
not_undercut(KB,Rule) :-
